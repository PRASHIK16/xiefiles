...
export default function UploadZone() {
  ...
  // in the same upload batch sees the SAME id, even mid-loop.
      ...
      try {
        ...
        if (xhr.status === 200 && res.success) {
          ...
          if (res.visibility === 'otp') {
            ...
            toast.success('Uploaded — visible to everyone on campus')
          }
        } else {
          toast.error(res.error || 'Upload failed. Check the file type and try again.')
        }
      } catch { toast.error('Server error — please try again in a moment.') }
      resolve()
    }
    xhr.onerror = () => {
      setProgress(null)
  ...
  const shareOtp = async () => {
    if (!batchRef.current || drafts.length === 0) { toast.error('Add at least one file first'); return }
    setSharing(true)
    try {
      ...
      if (data.success) {
        ...
        toast.success('Code ready — share it with anyone who needs the files')
      } else toast.error(data.error || 'Could not generate a code. Try again.')
  ...
  return (
    <div>
      {/* Mode toggle + Access with OTP — always visible, wraps cleanly on mobile */}
      <div className="mb-3">
        <div className="flex items-center gap-1.5 flex-wrap justify-between">
          <div className="flex items-center gap-1.5">
            <button onClick={() => setMode('public')}
              className={`chip transition-all active:scale-95 ${mode === 'public'
                ? 'bg-brand-500 text-white'
                : 'bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}>
              <Globe size={13} /> Public
            </button>
            <button onClick={() => setMode('otp')}
              className={`chip transition-all active:scale-95 ${mode === 'otp'
          ...
          {/* Secondary action — same /access route as the sidebar menu item */}
          <button
            onClick={() => navigate('/access')}
            className="chip border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900
                       text-slate-600 dark:text-slate-300 hover:border-brand-400 hover:text-brand-600
                       dark:hover:border-brand-500 dark:hover:text-brand-400
                       transition-all active:scale-95"
          >
            <KeyRound size={13} /> Access with OTP
          </button>
        </div>
      ...
      {isOtp && drafts.length > 0 && !otpResult && (
        <div className="card mt-3 p-4 animate-fadeUp">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={resetOtp} className="text-xs text-slate-400 hover:text-red-500 transition-colors">Clear</button>
          </div>
          ...
          <button onClick={shareOtp} disabled={sharing}
            className="btn btn-primary w-full justify-center disabled:opacity-50 transition-all active:scale-[0.99]">
            {sharing ? <><Loader2 size={16} className="animate-spin" /> Generating code…</>
      ...
      {otpResult && (
        <div className="card mt-3 p-5 animate-fadeUp border-brand-300 dark:border-brand-800 text-center">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={resetOtp} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 transition-colors" aria-label="Done">
              <X size={16} />
            </button>
          </div>
          ...
          <div className="flex items-center justify-center gap-2 mb-3">
            ...
            <button onClick={copyOtp} className="btn btn-ghost text-xs px-2.5 py-2 shrink-0 transition-all active:scale-90" title="Copy code">
              {copied ? <Check size={15} /> : <Copy size={15} />}
            </button>
          </div>
          <p className="text-[11px] text-amber-600 dark:text-amber-400">
            ⚠ Valid for 24 hours. To open the files, use "Access with OTP" and enter this code.
          </p>
        </div>
      )}
    </div>
